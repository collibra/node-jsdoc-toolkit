/* SeaJS v2.0-collibra-custom | seajs.com | MIT Licensed */
...
/**
 * @type {string} The version of the framework.
 */
seajs.version = '2.0-collibra-custom';
  ...
  util.serializeObject = function(obj) {
    ...
    for (var prop in obj) {
        ...
        if (Object.prototype.toString.call(obj[prop]) == '[object Object]') {
            pairs.push(util.serializeObject(obj[prop]));
            continue;
        }
...
(function(util, data, fn, global) {
  ...
  function id2Uri(id, refUrl) {
    var hasBang = id.indexOf('!') === 0, // Regular file request should have a bang.
        hasRoof = id.indexOf('^') === 0; // Region requests should have roof.

    if (hasRoof) {
      return id;
    }

    if (hasBang) id = id.substr(1);
    id = parseAlias(id);
    ...
    var normalized = normalize(ret);

    if (data.velocityCallbacks[id]) {
      data.velocityCallbacks[normalized] = data.velocityCallbacks[id];
      data.cachePolicies[normalized] = data.cachePolicies[id];
      data.params[normalized] = data.params[id];

      delete data.velocityCallbacks[id];
      delete data.cachePolicies[id];
...
(function(util, data, fn) {
  ...
  util.getAsset = function(url, callback, charset) {
    var hasBang = url.indexOf('!') === 0,
        hasRoof = url.indexOf('^') === 0,
        hash = '',
        url = hasBang || hasRoof ? url.substr(1) : url ;
    ...
    if (hasBang) {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
          var velo = this.responseText,
              indexClosingTag = velo.indexOf('<=========================='),
              includes = velo.substring(velo.indexOf('========INCLUDES==========>') + 27, indexClosingTag),
              velo = velo.substr(indexClosingTag + 27),
              includes = includes.split('\n'),
              velocityFunction = function() {},
              veloF = false;

          veloF = data.velocityCallbacks[id];
          if (veloF) (function() {    
            velocityFunction = function() {
              veloF(velo);
            };
          ...
          for (var i = 0; i < includes.length; i++) {
            ...
            //better testing for whitespaced/empty tokens
            if (!(/\S/g.test(item))) {
              includes.splice(i, 1);
              i = i - 1;
            } else {
      ...
      if (!hasRoof && data.cachePolicies[url] && data.cachePolicies[url].vm) {
        cache = '&id=' + Math.floor(Math.random() * 10000).toString();
      };

      if (page && page.resourceid) {
        rid = '/' + page.resourceid;
      }
      ...
      if (hasRoof) {
        ...
        xhr.open('POST', url + rid + '?bare=true' + cache, true);
      }

      var params = 'exclude-modules=' + data.modulesLoaded.join(','),
          paramsCall = data.params[id];

      if (paramsCall) {
        params = params + '&' + util.serializeObject(paramsCall);
  ...
  function assetOnload(node, callback) {
    ...
    var timer = setTimeout(function() {
      util.log('Time is out:', node.src || node.href);
      cb();
    }, data.config.timeout);
...
(function(util, data, fn) {
  ...
  function provide(uris, callback) {
    ...
    if (data.velocityCallbacks[uris[0]]) {
      unReadyUris.push(uris[0]);
    }
    ...
    for (var i = 0, n = unReadyUris.length, remain = n; i < n; i++) {
      (function(uri) {
        ...
        if (memoizedMods[uri] && !data.velocityCallbacks[uris[0]]) {
          onLoad();
        } else {
          if (data.velocityCallbacks[uris[0]]) delete fetchedList[uris[0]];
          fetch(uri, onLoad);
        }
  ...
  function fetch(uri, callback) {
    var hasBang = uri.indexOf('!'),
        modSrcUrl = uri,
        uri = hasBang === 0 ? uri.substr(1) : uri,
        srcUrl = util.parseMap(uri);
...
(function(host, data, fn) {
  ...
  fn.get = function(id, callback1, callback2, params, cachePolicy) {

    //Greg - fix for issue with including callback1(velocity callback)
    //when id is an alias instead of full path.
    if (data.config.alias.hasOwnProperty(id)) {
      id = data.config.alias[id];
    }

    data.velocityCallbacks[id] = callback1;
    data.cachePolicies[id] = cachePolicy;
    data.params[id] = params;

    fn.preload(function() {
      fn.load(id, callback2);
    });
  };

  fn.getRegion = function(id, callback1, params) {
    var fullRegion = "^" + window.location.toString().split('#')[0] + "?region=" + id;

    data.velocityCallbacks[fullRegion] = callback1;
    data.params[fullRegion] = params;

    fn.preload(function() {
      fn.load(fullRegion, function() {});
    });
  };
...
define('plugin-base', [], function(require, exports) {
  ...
  var util = !core ? seajs.pluginSDK.util : core.libs.get('seajs').pluginSDK.util ;
